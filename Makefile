.PHONY: server admin web dev test build migrate docker-up docker-down openapi-gen
...
migrate:
	cd server && $(GO) run ./cmd/server --help >/dev/null 2>&1 || true
	cd server && DATABASE_PATH=./data/app.db $(GO) run ./cmd/server
